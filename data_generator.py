...
def generate_data(n_customers: int = 2500, n_transactions: int = 40000, seed: int = 42):
    ...
    education = rng.choice(["Graduate", "Post-Graduate", "High School", "Doctorate"], size=n_customers, p=[0.48, 0.28, 0.18, 0.06])
    ...
    categories = np.array(["Groceries", "Travel", "Dining", "Shopping", "Utilities", "Fuel", "Entertainment", "Healthcare"])
    ...
    base_amount = {"Groceries": 70, "Travel": 220, "Dining": 90, "Shopping": 140, "Utilities": 110, "Fuel": 75, "Entertainment": 80, "Healthcare": 130}
    ...
    transactions = pd.DataFrame({
        ...
        "merchant": rng.choice(["Amazon", "Flipkart", "Reliance", "DMart", "MakeMyTrip", "Swiggy", "Zomato", "IndianOil", "Apollo", "BookMyShow"], size=n_transactions),
        "channel": rng.choice(["Online", "POS", "Contactless"], size=n_transactions, p=[0.47, 0.38, 0.15]),
        "transaction_amount": np.round(amounts, 2),
    })

    annual_spend = transactions.groupby("customer_id")["transaction_amount"].sum().reindex(customer_ids).fillna(0).to_numpy()
    utilization = np.clip((annual_spend / 12) / np.maximum(credit_limit, 1) + rng.normal(0.15, 0.08, n_customers), 0.03, 0.95)
    customers["outstanding_balance"] = np.round(credit_limit * utilization).astype(int)
    customers["utilization_ratio"] = np.round(customers["outstanding_balance"] / customers["credit_limit"], 4)
    customers["churn_risk_segment"] = np.where(
        (customers["customer_status"] == "Inactive") | (customers["utilization_ratio"] > 0.75) | ((customers["tenure_years"] <= 2) & (customers["utilization_ratio"] < 0.10)),
        "High",
        np.where((customers["utilization_ratio"] > 0.55) | (customers["tenure_years"] <= 3), "Medium", "Low"),
    )
...
def ensure_csvs(directory: Path):
    ...
    if customers_path.exists() and transactions_path.exists():
        return (
            pd.read_csv(customers_path),
            pd.read_csv(transactions_path, parse_dates=["transaction_date"]),
        )
